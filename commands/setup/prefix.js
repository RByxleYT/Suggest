...
module.exports = {
  ...
  run: async (client, message, args, user, text, prefix) => {
    if (!args[0]) return message.reply(`Current Prefix: \`${prefix}\` \nPlease provide a new prefix`)
        if (!message.member.hasPermission("ADMINISTRATOR")) return message.reply(`<:Decline:864080227738320896> You don\'t have permission for this Command!`)
        if (args[1]) return message.reply('<:Decline:864080227738320896> The prefix can\'t have two spaces')
        if(args.join(" ").length > 4) return message.reply("Thats too long! Maximum lenght is 4");
        client.settings.set(message.guild.id, args[0], `prefix`);
        return message.reply(`<:Accepted:864080263172718603> Successfully set new prefix to **\`${args[0]}\`**`)
